...
└─────────────────────────────────────────────────────────────┘
         ...
         └───────────┬───────────┘
                     ▼
         ┌─────────────────────────────────────┐
         │ Win32Window.click_screen(x, y)      │  ← 화면 좌표 직접 사용
         │                                     │
         │ → win32api.SetCursorPos((x, y))    │  ← 커서 이동
         │ → win32api.mouse_event(LEFTDOWN)   │  ← 하드웨어 레벨 클릭
         │ → win32api.mouse_event(LEFTUP)     │     (PostMessage보다 확실함)
         └─────────────────────────────────────┘

참고: PostMessage(WM_LBUTTONDOWN)은 가상 메시지로 일부 앱(KakaoTalk)에서
무시될 수 있어 mouse_event()를 사용하여 하드웨어 레벨 이벤트 생성.
```
...
└─────────────────────────────────────────────────────────────┘
         ...
         ┌─────────────────────────────────────┐
         │ 채팅 출력 영역 클릭                   │
         │ Win32Window.click_screen(x, y)      │  ← 하드웨어 레벨 클릭
         └───────────┬─────────────────────────┘
                     ▼
         ┌─────────────────────────────────────┐
         │ 전체 선택 (PostMessage)              │
