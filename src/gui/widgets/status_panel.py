...
class StatusPanel(ttk.LabelFrame):
    ...
    def __init__(self, parent):
        ...
        self.gold_label.grid(row=1, column=0, sticky="w", pady=2)

        self.gold_value = ttk.Label(self, text="0원", font=("", 12))
        self.gold_value.grid(row=1, column=1, sticky="e", pady=2)

        # Fail count display
        self.fail_label = ttk.Label(self, text="연속 실패:", font=("", 10))
        self.fail_label.grid(row=2, column=0, sticky="w", pady=2)

        self.fail_value = ttk.Label(self, text="0회", font=("", 12))
        self.fail_value.grid(row=2, column=1, sticky="e", pady=2)

        # Last result display
        self.result_label = ttk.Label(self, text="마지막 결과:", font=("", 10))
        self.result_label.grid(row=3, column=0, sticky="w", pady=2)

        self.result_value = ttk.Label(self, text="-", font=("", 12))
        self.result_value.grid(row=3, column=1, sticky="e", pady=2)
        ...
        self.status_label.grid(row=5, column=0, sticky="w", pady=2)

        self.status_value = ttk.Label(self, text="대기 중", font=("", 12))
        self.status_value.grid(row=5, column=1, sticky="e", pady=2)
